exports.format_time = function () {
    return function (text, render) {
        ...
        if (days === 1) {
            timestr = timestr + days.toString() + " " + global.day + " "; //" day ";
        } else if (days > 1) {
            timestr = timestr + days.toString() + " " + global.days + " "; //" days ";
        }
        if (hours === 1) {
            timestr = timestr + hours.toString() + " " + global.hour + " "; //" hour ";
        } else if (hours > 1) {
            timestr = timestr + hours.toString() + " " + global.hours + " "; //" hours ";
        }
        if (mins === 1) {
            timestr = timestr + mins.toString() + " " + global.min + " "; //" minute ";
        } else if (mins > 1) {
            timestr = timestr + mins.toString() + " " + global.mins + " "; //" minutes ";
        }
        if (days < 1) {
            if (secs === 1) {
                timestr = timestr + secs.toString() + " " + global.sec; //" second ";
            } else if (secs > 1) {
                timestr = timestr + secs.toString() + " " + global.secs; //" seconds ";
            }
        }
